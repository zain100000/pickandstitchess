...
const LadiesOrders = () => {
  ...
  const getApiData = async () => {
    const url = "https://pickandstitches-backend.onrender.com/api/ladies?";

    try {
      const response = await axios.get(url);
      const result = response.data.LadiesOrder; // Access GentsOrders from the response data
      setData(result);
    } catch (error) {
  ...
  const handleIndividualDelete = async (id) => {
    ...
    if (userConfirmed) {
      try {
        ...
        await axios.delete(
          `https://pickandstitches-backend.onrender.com/api/ladies/${id}`
        );
